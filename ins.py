...
try:
    ...
    elements_entered = 0
    desc_rows = {}  # نگهداری ردیف هر شرح

    # ---------- وارد کردن داده‌ها در اکسل ----------
    for desc in descriptions:
        if desc in desc_rows:
            # شرح تکراری → جمع کردن تعداد
            row = desc_rows[desc]
            prev_tedad = ws[f"{out_cols['tedad']}{row}"].value
            prev_tedad = int(prev_tedad) if prev_tedad else 0
            ws[f"{out_cols['tedad']}{row}"].value = prev_tedad + 1
        else:
            row = row_start + elements_entered
            desc_rows[desc] = row
            ...
            ws[f"{out_cols['tedad']}{row}"].value = 1  # تعداد پیشفرض = 1
    ...
    ws[f"{out_cols['vazn']}{sum_row}"].value = f"=SUM({out_cols['vazn']}{row_start}:{out_cols['vazn']}{sum_row-1})"
